...
@media(max-width:1100px){ .hof-grid{ grid-template-columns:repeat(2,1fr); } }
@media(max-width:560px) { .hof-grid{ grid-template-columns:1fr; } }
.hof-card {
  padding:16px 18px; display:flex; flex-direction:column; gap:6px;
  transition:transform .15s, border-color .15s;
}
.hof-card:hover { transform:translateY(-2px); border-color:var(--line-hi); }
.hof-rank {
  position:absolute; top:14px; right:16px;
  font-family:'Space Grotesk',sans-serif; font-size:.8rem; font-weight:700; color:var(--muted);
}
.hof-rank.r1 { color:var(--gold); } .hof-rank.r2 { color:#cbd5e1; } .hof-rank.r3 { color:#b45309; }
.hof-score { font-family:'Space Grotesk',sans-serif; font-size:2rem; font-weight:700; line-height:1; }
.hof-player{ display:flex; align-items:center; gap:6px; font-size:.82rem; font-weight:600; }
.hof-course{ font-size:.72rem; color:var(--text-dim); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.hof-sub   { font-size:.66rem; color:var(--muted); }
...
/* ── Deep metrics 4-col grid ─────────────────────────────── */
.deep-metrics-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:14px; }
@media(max-width:1100px){ .deep-metrics-grid{ grid-template-columns:repeat(2,1fr); } }
@media(max-width:600px) { .deep-metrics-grid{ grid-template-columns:1fr; } }

/* ── Tooltip ─────────────────────────────────────────────── */
.has-tip { position:relative; display:inline-flex; align-items:center; gap:5px; }
.tip-icon { width:13px; height:13px; border-radius:50%; background:rgba(255,255,255,0.1);
  color:var(--muted); font-size:0.6rem; font-weight:700; display:grid; place-items:center;
...
.has-tip .tip-box {
  ...
  background:var(--panel); border:1px solid var(--line-hi); border-radius:8px;
  padding:9px 11px; font-size:0.71rem; color:var(--text-dim); line-height:1.5;
  width:230px; z-index:200; pointer-events:none; white-space:normal;
...
<div class="app">
  ...
  <main>
    ...
    <section aria-labelledby="hof-title">
      <div class="section-title" id="hof-title">
        <span class="st-accent">Hall of Fame</span>
        <span class="panel-sub" style="margin-left:auto;text-transform:none;letter-spacing:0">4 lowest rounds all-time</span>
      </div>
      <div class="hof-grid" id="hallOfFame"></div>
    </section>
...
function renderHallOfFame() {
  ...
  let qualifying = ROUNDS.filter(r => r.holeCount === 18 && r.score != null);
  if (activePlayer !== 'all') qualifying = qualifying.filter(r => r.player === activePlayer);

  const top = [...qualifying]
    .sort((a,b) => (a.score - b.score) || ((a.net ?? 999) - (b.net ?? 999)))
    .slice(0, 4);

  if (!top.length) {
    el.innerHTML = '<div style="color:var(--muted);font-size:.82rem;padding:16px 0">No rounds yet.</div>';
    return;
  }
  ...
  el.innerHTML = top.map((r, i) => {
    ...
    const col = COLORS[r.player] || 'var(--accent)';
    ...
    return `<div class="panel hof-card">
      <span class="hof-rank ${rankClass}">${String(rank).padStart(2,'0')}</span>
      <div class="hof-score" style="color:${col}">${r.score}</div>
      <div class="hof-player">
        <span class="player-dot" style="background:${col};color:${col}"></span>
        <span style="color:${col}">${r.player}</span>
      </div>
      <div class="hof-course" title="${r.course}">${r.course}</div>
      <div class="hof-sub">${dateStr}${r.net != null ? ` · net ${r.net}` : ''}</div>
    </div>`;
  }).join('');
}
...
function renderDeepDiveStats() {
  ...
  const players = rankedPlayers();
  const stats = [
    { key:'std10',           label:'Volatility Index',      tip:'Standard deviation of scores over the last 10 18H rounds. Lower = more consistent.', suffix:' σ',       lower:true },
    { key:'blowup_recent18', label:'Blow-up Rate',          tip:'Average double bogeys or worse per round, calculated over your last 5 rounds.',       suffix:' / round', lower:true },
    { key:'birdies_recent18',label:'Birdie+ Rate',          tip:'Average birdies and eagles per round, calculated over your last 5 rounds.',           suffix:' / round', lower:false },
    { key:'sim_index',       label:'Simulator Index',       tip:'Handicap-style estimate. Differential per round = (score − rating) × 113 ÷ slope. Best 8 of last 20 differentials, averaged × 0.96. Requires rating/slope data; not an official handicap.', suffix:'', lower:true },
  ];
  const panels = stats.map(st=>{
    const rows = players
    ...
    const labelHtml = `<span class="has-tip" style="font-size:0.7rem;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:var(--muted)">${st.label}<span class="tip-icon">?</span><span class="tip-box">${st.tip}</span></span>`;
    return `<div class="panel shot-bar-section"><div style="margin-bottom:8px">${labelHtml}</div>${rows || '<div class="no-data">Not enough data</div>'}</div>`;
  });
  el.innerHTML = `<div class="deep-metrics-grid">${panels.join('')}</div>`;
}
...
function renderInsights() {
  ...
  if (activePlayer==='all') {
    ...
    if (byIndex[0]) cards.push(insightCard('ruler','Lowest Sim Index',byIndex[0],fmt1(PER_PLAYER[byIndex[0]].sim_index),`Best differentials, last 20 rounds`,'var(--accent)','Handicap-style estimate. Differential per round = (score − rating) × 113 ÷ slope. Best 8 of last 20 differentials, averaged × 0.96. Requires rating/slope data; not an official handicap.'));
    ...
    if (pp.sim_index!=null) cards.push(insightCard('ruler','Simulator Index',fmt1(pp.sim_index),`Handicap-style estimate`,`Uses rating/slope data; not official`,col,'Differential per round = (score − rating) × 113 ÷ slope. Best 8 of last 20 differentials, averaged × 0.96. Requires rating/slope data; not an official handicap.'));
...
function insightCard(icon, label, main, sub, extra, color='var(--accent)', tip='') {
  if (!main) return '';
  const safeColor = color.startsWith('#') ? color : color;
  const isHex = color.startsWith('#');
  const borderTint = isHex ? hexAlpha(color, 0.30) : 'var(--line-hi)';
  const labelHtml = tip
    ? `<span class="has-tip">${label}<span class="tip-icon">?</span><span class="tip-box">${tip}</span></span>`
    : label;
